...
- **A database with the project, optionally**: tick a box in New Project, pick MySQL or PostgreSQL and a name, and its credentials are written straight into that project's env in time for the first `php artisan migrate`. A `.sql` dump can be attached to the same step and is replayed into the new database (with the engine's own `mysql`/`psql` client, as that database's user) before the first deploy runs.
...
- **Databases**: create/drop MySQL and Postgres databases with a dedicated user, reveal credentials on demand, inject them straight into a project's env. Each project has its own **Database** tab listing the databases linked to it, with host/port/username on the page and the password and `DB_*` env block revealed on request.
- **Workers and cron**: background worker processes (systemd) and a managed crontab block, both editable per project. Workers are configured entirely from the dashboard — process count, start-on-boot, restart policy and delay, and the grace period a worker gets to finish its current job on shutdown — with stack-aware presets for the usual cases (Laravel queue/Horizon, a Node worker script).
- **Move a project to another subdomain**: the Settings tab's Domain card changes the address a project answers on — Shipway points a DNS record for the new subdomain at this server, re-renders the nginx vhost, removes the old record, and rewrites the old domain wherever it appears in the project's environment variables. The project's internal name (its app directory, units and logs) is untouched, and a failed move leaves it exactly where it was.
...
- **Per-project SMTP** with Mailpit as the default catch-all inbox, a project's own SMTP credentials, or Amazon SES (pick a region and paste your SES SMTP credentials; the endpoint is derived and written into the project's `.env`).
- **Instance mail**: a separate mail configuration (Settings > Mail) Shipway itself uses to email invite links and route notification events to an address — the local Mailpit catch-all, your own SMTP server, or Amazon SES (pick a region and paste your SES SMTP credentials; the endpoint is derived for you) — with a test-send to confirm it before relying on it.
...
- **Per-project access**: a Member can be scoped to specific projects at invite time (or afterwards, from Settings > Team) instead of the whole instance. Enforced server-side across every project-scoped route, the deploy logs, the databases list and the pgAdmin console — not just hidden in the UI. Existing members default to all projects, so upgrading changes nothing until you scope someone.
- **Per-project deploy notifications**: each project keeps its own list of email recipients and picks which deploy events they hear about (failed, succeeded, canceled, rolled back), configured in the project's Settings tab with a test-send from the dashboard. Delivery goes through instance mail.

...
Deploys run as `deployer`, but nginx serves PHP through php-fpm as `www-data`, so an app that writes at runtime (Laravel's `storage/`, above all) needs both users to have access. After the build and before the release goes live, Shipway grants `www-data` **and** `deployer` read/write on every shared path and on the release's `bootstrap/cache`, using POSIX ACLs with `default:` entries so files created later — by `www-data` at runtime, or by `deployer` on the next deploy — stay accessible to the other. It shows up in the deploy log as the `permissions` stage. If `setfacl` fails the deploy still goes out, with a warning in that log; an app that turns out to need it will otherwise fail its first request with `Failed to open stream: Permission denied`.

The `deployer` half matters as much as the `www-data` half, and for a less obvious reason: the project's own queue workers and cron entries run as `deployer`, while `storage/logs/laravel.log` is typically created by the first *web* request, as `www-data:www-data` under php-fpm's `022` umask. Directory ownership grants nothing over a file inside it, so without an ACL entry the workers cannot append to that log — `artisan queue:work` dies on its first log line, restarts, and dies again, forever.

A deploy can only set an ACL on a file it owns, so it cannot repair such a file that already exists. `sudo /opt/shipway/setup/repair-app-acls.sh` does, as root, for every PHP project on the host; `update.sh` and `deploy-local.sh` both run it, so an ordinary update repairs the host. It is idempotent (`setfacl` skips a file whose ACL already says what it is being asked to say) and touches only directories a PHP deploy has already granted `www-data` — a node or static project's shared directory is left alone.
